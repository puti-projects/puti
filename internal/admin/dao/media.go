...
// GetTotalMedia get total number of media
func (d *Dao) GetTotalMedia() (totalMedia int64) {
	var err error

	media := &model.Media{}
	totalMedia, err = media.TotalNumber(d.db)
	if err != nil {
		logger.Errorf("statistics error: get total media failed, %v", err)
	}
